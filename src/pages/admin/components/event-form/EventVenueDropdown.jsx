...
export function EventVenueDropdown({ venues, value, onChange, required }) {
  ...
  return (
    <div className="relative flex flex-col gap-1.5" ref={containerRef}>
      ...
      <p className="text-[10px] text-slate-400">Where will the event be held?</p>
      ...
      >
        <span className={selectedVenue ? "font-medium text-slate-800" : "text-slate-400"}>
      ...
      <AnimatePresence>
        {open ? (
          ...
          >
            {venues.length === 0 ? (
              <p className="px-4 py-3 text-sm text-slate-400">No venues available</p>
            ) : venues.map((venue) => (
              <button
                key={venue.id}
                type="button"
                ...
                className={`flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm transition-colors ${venue.id === value ? "bg-primary/10 font-semibold text-primary" : "text-slate-700 hover:bg-slate-50"}`}
              >
                ...
                <div className="min-w-0">
                  ...
                  {venue.location ? <p className="truncate text-[11px] text-slate-400">{venue.location}</p> : null}
                </div>
                {venue.id === value ? <div className="ml-auto h-2 w-2 shrink-0 rounded-full bg-primary" /> : null}
              </button>
            ))}
          </motion.div>
        ) : null}
